...
Future<int> createOrderFunction(
  ...
  if (_selectedItems.isEmpty ||
      (paymentMethod == 2 && price > customer.tokens && role != 'worker')) {
    // Notify user something is wrong with order parameters.

    String message = '';
    if (_selectedItems.isEmpty) {
      message = AppStringValues.chooseItemsDot;
    } else if (paymentMethod == 2 && price > customer.tokens) {
    ...
    if (role == 'worker') {
      ...
      _docRef = await CompanyOrderDatabase().createPassiveOrder(status, stringList, price,
          pickUpTime, username, shop, company, orderID, userID, shopID, companyID, day);
    } else {
    ...
    for (Product item in _selectedItems) {
      print(item.name);
      await ProductDatabase().updateProductData(item.productID, item.count + 1);
    }
    ...
    if (role == 'customer') {
      if (paymentMethod == 1) {
        ...
        try {
          customer.updateTokens(customer.tokens - price);
